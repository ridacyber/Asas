...
function lex(source) {
  ...
  while (i < source.length) {
    ...
    if (isDigit(c)) {
      ...
      tokens.push({ type: TokenType.NUMBER, value: parseFloat(source.slice(start, i)) });
      continue;
    }
    ...
    if (isAlpha(c)) {
      ...
      if (word === "let") tokens.push({ type: TokenType.LET });
      else if (word === "print") tokens.push({ type: TokenType.PRINT });
      else tokens.push({ type: TokenType.IDENT, value: word });
      continue;
    }
    ...
    switch (c) {
      ...
      case "*": tokens.push({ type: TokenType.STAR }); i++; break;
      case "/": tokens.push({ type: TokenType.SLASH }); i++; break;
      case "=": tokens.push({ type: TokenType.EQUALS }); i++; break;
      case "(": tokens.push({ type: TokenType.LPAREN }); i++; break;
      case ")": tokens.push({ type: TokenType.RPAREN }); i++; break;
      case ";": tokens.push({ type: TokenType.SEMI }); i++; break;
      default:
...
function parse(tokens) {
  ...
  function parseTerm() {
    ...
    while (peek().type === TokenType.STAR || peek().type === TokenType.SLASH) {
      const op = advance().type === TokenType.STAR ? "*" : "/";
...
export default function AsasIDE() {
  ...
  function handleRun() {
    ...
    try {
      const output = runAsas(code);
      const newLines = output.length
